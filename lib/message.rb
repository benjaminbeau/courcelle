...
class Flower::Message
  ...
  def respond?
    event =~ /message|comment/ && @flows.keys.include?(flow)
  end
  ...
  def reply(reply, options = {})
    reply = reply.respond_to?(:join) ? reply.join("\n") : reply
    @output = reply
    rest.post_message("@#{nick}: #{reply}", parse_tags(options), flow)
  end
  ...
  def send_to_inbox(reply, options = {})
    reply = reply.respond_to?(:join) ? reply.join("\n") : reply
    reply.gsub!(/\n/, "<br>")
    rest.post_to_inbox(reply, parse_tags(options), options[:subject], flow)
  end
